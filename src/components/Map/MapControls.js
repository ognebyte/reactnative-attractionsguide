    ...
    return (
        <>
            ...
            <View style={{ position: 'absolute', right: padding, bottom: padding, alignItems: 'center' }}>
                <IconButton mode='contained' icon={() => (
                        <Svg width="90%" height="90%" viewBox="0 0 24 24" fill="none">
                            <Path d="M12 2L9 12H15L12 2Z" fill="#FF0000" />
                            <Path d="M15 12L11.9908 22L9 12H15Z" fill="black" />
                        </Svg>
                    )}
                    size={24} onPress={onResetToNorth}
                    style={{ transform: [{ rotate: `${360 - heading}deg` }] }}
                />
                <IconButton mode='contained' icon={gpsEnabled ? "crosshairs-gps" : "crosshairs-question"}
                    size={30} onPress={onGoToCurrentLocation}
                />
            </View>
            ...
            <View style={{ position: 'absolute', right: padding, top: '50%' }}>
                <IconButton mode='contained' icon='plus'
                    size={30} onPressIn={onZoomIn}
                    onLongPress={() => onStartZoom(true)}
                    onPressOut={onStopZoom}
                />
                <IconButton mode='contained' icon='minus'
                    size={30} onPressIn={onZoomOut}
            ...
            {/* Map options toggle */}
            <IconButton mode='contained' icon={mapOptionsVisible ? 'layers' : 'layers-outline'}
                size={30} onPress={onMapOptionsPress}
